...
[基本使用](codes/6.函数/7.niming.py ':include :type=code :fragment=jbsy')

**与高阶函数结合**

[与高阶函数结合](codes/6.函数/7.niming.py ':include :type=code :fragment=gjhs')

**在函数内使用匿名函数**

有时候，在定义函数时，如果需要短暂使用一个辅助函数，而这个辅助函数很简单，使用匿名函数会非常方便。

[在函数内使用匿名函数](codes/6.函数/7.niming.py ':include :type=code :fragment=fzhs')
